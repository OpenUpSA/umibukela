...
class AdminSite(admin.AdminSite):
    ...
    def get_urls(self):
        ...
        urls = [
            url(r'^umibukela/cycle/(?P<cycle_id>\d+)/create_zip$',
                'umibukela.views.create_materials_zip',
                name='cycle-create-zip'),
            url(r'^umibukela/kobo_forms/(?P<kobo_form_id>\d+)/site/(?P<site_name>\w+)/preview$',
                self.admin_view(views.kobo_form_site_preview)),
            url(r'^umibukela/survey/(?P<survey_id>\d+)/kobo_submissions$',
                self.admin_view(views.survey_kobo_submissions)),
            url(r'^umibukela/programme/(?P<programme_id>\d+)/kobo_grant$',
                self.admin_view(views.programme_kobo_grant),
                name='programme-kobo-grant'),
            url(r'^umibukela/survey_from_kobo$',
...
class SurveyTypeAdmin(admin.ModelAdmin):
    ...
    def change_view(self, request, id, extra_context=None):
        ...
        return super(SurveyTypeAdmin, self).change_view(
            request, id, extra_context=extra_context)
...
class CycleResultSetAdmin(admin.ModelAdmin):
    ...
    list_filter = ('survey__cycle__programme', 'survey__type',
                   'site__province', 'funder', 'survey__cycle')
...
class CycleAdmin(admin.ModelAdmin):
    def change_view(self, request, id, extra_context=None):
        ...
        return super(CycleAdmin, self).change_view(
            request, id, extra_context=extra_context)
...
admin_site.register(ProgrammeImage, ProgrammImageAdmin)
